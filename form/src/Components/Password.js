...
function Create({ text, message, id, value, change, bg }) {
    return (
        <form className="options">
            <label htmlFor="inputPassword5" className="form-label bold password" style={bg}>{text}</label>
            <input type="password" id={id} className="form-control" aria-describedby="passwordHelpBlock" value={value} onChange={change} />
            <div className="form-text" style={bg} >{message}
            </div>
        </form>
    );
}
